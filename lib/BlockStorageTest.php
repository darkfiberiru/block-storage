    ...
    if (is_dir($dir) && is_writable($dir) && mkdir($tdir = sprintf('%s/%d', $dir, rand())) && ($fp = fopen($htmlFile = sprintf('%s/index.html', $tdir), 'w'))) {
      ...
      if (filesize($htmlFile)) {
        ...
        if (!isset($options['nopdfreport']) || !$options['nopdfreport']) {
          // generate PDF report
          $cmd = sprintf('cd %s; %swkhtmltopdf --enable-local-file-access -s Letter --footer-left [date] --footer-right [page] --footer-font-name rfont --footer-font-size %d index.html report.pdf >/dev/null 2>&1; echo $?', $tdir, isset($options['wkhtml_xvfb']) ? 'xvfb-run ' : '', $options['font_size']);
          $ecode = trim(exec($cmd));
          sleep(1);
  ...
  public final function purge() {
    ...
    if (!$nopurge) {
      foreach($this->options['target'] as $target) {
        ...
        if (!$purged && !$notrim) {
          if ($this->deviceTargets) {
             $cmd =  sprintf('fio --name=trim --bs=10g --rw=trim %s --filename %s >/dev/null 2>&1; echo $?', false && $this->deviceTargets && isset($this->options['trim_offset_end']) && $this->options['trim_offset_end'] > 0 ? sprintf(' -o 0 -l %d', self::getFreeSpace($target, TRUE) - $this->options['trim_offset_end']) : '', $this->deviceTargets ? $target : $volume);
          }        
          else {
             $cmd = sprintf('fstrim %s %s >/dev/null 2>&1; echo $?', $this->deviceTargets && isset($this->options['trim_offset_end']) && $this->options['trim_offset_end'] > 0 ? sprintf(' -o 0 -l %d', self::getFreeSpace($target, TRUE) - $this->options['trim_offset_end']) : '', $this->deviceTargets ? $target : $volume);
          }
